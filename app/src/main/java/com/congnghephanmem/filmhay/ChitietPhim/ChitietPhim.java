...
public class ChitietPhim extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        id = intent.getStringExtra("idf");
        img = (ImageView) findViewById(R.id.img_chitietphim);
        chitietphim  =(RelativeLayout) findViewById(R.id.chitietphim);

        /*try {
            Integer.parseInt(intent.getStringExtra("img").trim());
            Picasso.get().load(intent.getStringExtra("img").trim()).into(img);
        }catch (Exception e){
            Picasso.get().load(intent.getStringExtra("img").trim()).into(img);
...
//                   bl = new binhluan("Người dùng",txt_comt.getText().toString().trim(),System.currentTimeMillis());
                    ...
                    reference.child("comment").child(intent.getLongExtra("id",0)+"").push().setValue(bl);
                }
    ...
    void get_data_cmt(){
        reference.child("comment").child(intent.getLongExtra("id",0)+"").addChildEventListener(new ChildEventListener() {
            @Override
